...
> 附言：2020年末以来，国际MCU市场遇到了史无前例的芯片危机，并且严重影响到了各大依赖MCU的产业，比如汽车工业，家电，以及自动化领域。在中国国内，各大代理以及销售商开始借机大量囤货，导致单片机、FPGA价格大幅上涨，这在低端产品中尤为严重，例如ST的STM32F103C8T6价格涨幅最高超过了1000%。据业内不可靠预计这场芯片危机将会持续两年。
>
> 物贵则惜。本环境的可用性，会尽量通过手边已有的开发板验证，并且编写或适配相应的Makefile
...
| 命令 | 作用 |
| :-: | :-: |
| `packihx` | 将`.ihx`转换为标准`.hex`文件，输出到标准输出（可以不用转换，直接烧录`.ihx`文件） |
...
SDCC扩展的关键词如下，**分为变量声明和寄存器声明两种**，和Keil C51类似，区别是在前面都要加上`__`双下划线
...
寄存器常量通过`__at`关键词进行规定，这和Keil C51不同，后加寄存器所在地址常量，**其实就是给一个地址处的寄存器规定一个符号，之后可以将这个符号赋值给一个寄存器变量（本质是将一个地址赋值给一个寄存器变量）**。一般寄存器常量已经在对应头文件中有声明，用户无需操作。**16bit寄存器需要分别指定高位和低位地址（如果寄存器访问有顺序最好不要使用__sfr16声明，因为有可能会乱序）**，示例如下
...
其中`__interrupt`关键词使得编译器在中断向量表中插入到这个函数的跳转指令（这里是中断0），`__using`关键词可以用于指定这个中断使用的工作寄存器组（0\~3），这样在保存现场时不用压栈所有通用寄存器，只要压栈ACC、B等关键寄存器即可（也可以不指定，但是会导致中断响应延迟增加）
...
> 4. 尽量不要在中断中或有中断发生的main中使用不可重入函数，这包括一些需要外部浮点函数库的数学运算。如果要使用，最好将这些不可重入函数使用--stack-auto重新编译，并且源代码需要使用--int-long-reent重新编译


### 2.2.4 常用技巧

**由于8051的体系结构限制，所有IO寄存器（SFR）只能使用直接寻址方式，所以不能使用IO引脚作为函数的参数**（因为所有访问IO的指令，比如设置P0`MOV 80H, #01H`在编译时就已经定死无法更改），也不存在IO变量，这极大限制了8051代码的灵活性以及代码的重复使用
...
因为编译器处理流程是预处理-编译-汇编-链接，所以如果是自己设计的函数，头文件分别放在./inc以及./src下，宏定义也必须放在相应头文件中而不是main.c中（可以通过在用户区添加一个共用头文件实现，但是需要修改Makefile，目前的Makefile较为简单还无法实现qwq，计划改进Makefile支持更加复杂的文件依赖，或者引入自动依赖）

从软件工程的角度看，使用宏定义的方案不是一种完美的代码复用方案，因为需要更改原来的代码再次编译，这也不利于库文件以及可重定位文件的应用，因而8051开发中库文件一般只用在不涉及IO的场合。但是使用宏定义效率最高
...
如果使用函数指针，需要定义额外的函数（可以额外定义一套.c和.h）用以不同的IO读写，消耗程序空间，并且函数调用也会额外增加开销，如果增加判断语句会进一步降低效率，综合来看效率不如宏定义，并且代码繁琐。这种方案一般很少采用
