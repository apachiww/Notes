...
> 平行视图校正分为5步
>
> 1. 在两张图像中匹配$\ge 8$对点$p_{li},p_{ri}$（使用图像匹配算法），**计算$F$**
> 2. 由于所有极线相交于极点，所以可以先由$u_l = F^Tp_r,u_r = Fp_l$计算出各点所在极线，再通过$\begin{bmatrix} u_{l1} \\ u_{l2} \\ \vdots \\ u_{li} \end{bmatrix} e_l = 0$代入通过SVD**求得极点**$e_l$，而$e_r$同理
> 3. **求解$H_r$**，将$e_r$映射到无穷远$(f,0,0)$。$H_r = T^{-1}GRT$分为如下几个方面
> **首先将右图像坐标原点平移到图片中心**
...
> **最后通过$T^{-1}$将坐标还原**
> 4. 需要使得$\Pi_r$经过$H_r$变换的图像和$\Pi_l$经过$H_l$变换的图像重合越多越好，使得
> $$ \sum_i d(H_lp_{li}, H_rp_{ri}) $$
> **值最小**
> 5. 最后使用求得的$H_lH_r$对左右图像进行重采样转换
