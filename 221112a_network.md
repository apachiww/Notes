...
内置的根证书位于`/etc/ssl/certs`，也是`x509`格式，但是使用`xxxxxxxx.0`的文件名。`xxxxxxxx`是一个哈希值，可以通过以下命令生成
...
目前IETF成立的`QUIC`小组正在致力于`QUIC`的标准化，以便于未来的广泛应用。除RFC9000系列已完成外，其余有很多文档依旧在完善中
...
服务器回复的`Initial`中`DCID`需要设置为客户端给出的`SCID`

接收到服务器回复的`Initial`或`Retry`后，客户端数据包的`DCID`才会确定（服务器可能会更改自己的`SCID`），客户端**必须**将后续**发送**数据包的`DCID`改为服务器回复的`SCID`。后续的数据传输只能使用`NEW_CONNECTION_ID`更新CID
...
> 目前很多的`QUIC`实现并不完全符合该条，这些参数可能缺失。例如下述`original_destination_connection_id`参数，在服务器不更改原有CID的情况下可能不会发送该参数
>
> 以下需要结合上文**连接ID：协商**理解

`QUIC`规定通信双方发送的第一个`Initial`数据包中，需要有一个参数`initial_source_connection_id`，其值就是该数据包的`SCID`；而服务器回复的第一个`Initial`数据包还需要包含一个`original_destination_connection_id`，其值就是先前客户端发来的`Initial`中`DCID`，也就是先前说的最少`8`字节的CID（该数据包的`SCID`和`initial_source_connection_id`可能和该值不同）。如果此时服务器想要发送`Retry`而不是回复`Initial`，除以上参数还需要包含`retry_source_connection_id`，其值为该`Retry`数据包的`SCID`
...
> 之后服务器回复的`UDP`数据包中会有两个长数据头类型的`QUIC`数据包，分别为`Initial`（明文的`Server Hello`）以及`Handshake`；此外还有一个`1-RTT`数据包，共计3个数据包共用一个`UDP`。其中`Handshake`中的数据全部加密，所以显示为黄色。我们看到服务器的`Initial`数据包中包含了`ACK`（`ACK`了客户端的`Initial`数据包，包序号`1`）。服务器回复的`Initial`中`CRYPTO`只有很少的一些扩展参数
