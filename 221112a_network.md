...
>
> 由于该`TCP`传输被配置为偏实时性任务，该丢失的数据包最终还是超时后通过**快速重传**传输成功
...
>
> 实际应用中路由以及交换等操作会引入比上述大的多的延迟，所以`65535`字节的窗口是不够的，只能启用Window Scaling扩展来规避瓶颈
...
`QUIC`（发音同quick）基于`UDP`设计，最初由Google开发，作为`TCP`的替代品，主要用于`HTTP`。现在主流浏览器都已支持`QUIC`
