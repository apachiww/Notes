...
> 数据包接收方的**网络层**一旦发现`ECN`为`0b11`，它会通知上层的`TCP`将下一次回复的`TCP`数据包中`ECE`标志位置`1`再发往原先的数据发送方，发送方才能知道发生拥塞。**此时数据接收方和发送方会采用对待丢包一样的拥塞调节措施**（通常是[降窗快速恢复](#515-tcp拥塞控制特性)），这通常意味着窗口的减小，接收方发送`TCP`数据包中的`Window`值会减小
>
...
> 由于`QUIC`支持一个`UDP`数据包中封装多个数据包，且不同类型的数据包可能会使用不同的数据包序号空间。对于`ECN`有效的`UDP`数据包中每一个`QUIC`数据包，其对应的数据包序号空间的`Count`都要`+1`
>
...
目前`TCP TLS`协议栈最常用`TLS 1.2`或`TLS 1.3`。IETF强制要求`QUIC`使用`TLS 1.3`，但是目前仍有一些会使用`TLS 1.2`
...
>
> 目前（2023Q1）`QUIC`依旧处于[重复造轮子](https://github.com/quicwg/base-drafts/wiki/Implementations)的阶段，有多种实现且集成于各应用中，例如符合IETF标准的就可以称为`IETF QUIC`，而Google开发的版本就称为`Google QUIC`。现在的`IETF QUIC`相比最早的`Google QUIC`已经有了较多的改动。**很多现有的实现并不符合IETF的规范，各个实现的行为也不尽相同，在之后的学习以及实验中需要注意**。使用Wireshark抓取不同软件的`QUIC`包可以观察发现许多的差异
>
