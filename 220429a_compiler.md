...
```cpp
int a = 0;				// 符号a属于.bss节，STB_GLOBAL，STT_OBJECT
int b;					// 符号b属于COMMON伪节，STB_WEAK，STT_OBJECT。此时b由于未初始化所以为弱符号，可以被其他文件中已初始化的同名强符号覆盖
int c = 3;				// 符号c属于.data节，STB_GLOBAL，STT_OBJECT
...
int main() {			// 符号main属于.text节，STB_GLOBAL，STT_FUNC
	...
	int j;				// 变量j在栈区，不会出现在符号表。此时j为不确定值，因为栈区不像.bss节那样初始化为0
	int k[64];			// 数组变量k在栈区，不会出现在符号表。内容为不确定值
	// 在程序运行时使用malloc()分配的空间在堆区管理
	return 0;
}

// 看完上面的代码注释你还觉得自己了解C吗（小声）
```
