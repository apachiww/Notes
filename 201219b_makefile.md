...
+ [**0**](#0-空降) 空降
+ [**1**](#1-简介) 简介
    + [**1.1**](#11-make是什么) Make是什么
    + [**1.2**](#12-gnu-gcc工具链) GNU GCC工具链
+ [**2**](#2-make执行流程) Make执行流程
    + [**2.1**](#21-两个阶段) 两个阶段
    + [**2.2**](#22-makefile的解析过程) Makefile的解析过程
+ [**3**](#3-makefile格式以及编写) Makefile格式以及编写
    + [**3.1**](#31-makefile变量) Makefile变量
        + [**3.1.1**](#311-变量的赋值) 变量的赋值
        + [**3.1.2**](#312-内建特殊变量) 内建特殊变量
        + [**3.1.3**](#313-自动变量automatic-variables) 自动变量（Automatic Variables）
    + [**3.2**](#32-规则rule) 规则（Rule）
        + [**3.2.1**](#321-规则基本格式) 规则基本格式
        + [**3.2.2**](#322-依赖prerequisites类型) 依赖（prerequisites）类型
        + [**3.2.3**](#323-使用通配符wildcard) 使用通配符（wildcard）
        + [**3.2.4**](#324-伪目标phony) 伪目标（Phony）
        + [**3.2.5**](#325-目录搜索) 目录搜索
        + [**3.2.6**](#326-内建特殊目标) 内建特殊目标
        + [**3.2.7**](#327-单规则多目标) 单规则多目标
        + [**3.2.8**](#328-单目标多规则) 单目标多规则
        + [**3.2.9**](#329-静态格式规则static-pattern-rules) 静态格式规则（Static Pattern Rules）
        + [**3.2.10**](#3210-双冒号规则) 双冒号规则
        + [**3.2.11**](#3211-使用编译器自动获取依赖) 使用编译器自动获取依赖
    + [**3.3**](#33-隐式规则implicit-rules) 隐式规则（Implicit rules）
    + [**3.4**](#34-构建操作recipe) 构建操作（Recipe）
        + [**3.4.1**](#341-recipe基本格式) recipe基本格式
        + [**3.4.2**](#342-recipe中的变量) recipe中的变量
        + [**3.4.3**](#343-recipe调用shell的方式) recipe调用shell的方式
        + [**3.4.4**](#344-recipe的并行执行) recipe的并行执行
        + [**3.4.5**](#345-中断make) 中断make
    + [**3.5**](#35-条件判断) 条件判断
    + [**3.6**](#36-使用自带函数) 使用自带函数
        + [**3.6.1**](#361-字符串函数) 字符串函数
        + [**3.6.2**](#362-文件名函数) 文件名函数
        + [**3.6.3**](#363-foreach函数) foreach函数
        + [**3.6.4**](#364-shell函数) shell函数
        + [**3.6.5**](#365-file函数) file函数
        + [**3.6.6**](#366-call函数) call函数
        + [**3.6.7**](#367-警告函数) 警告函数
+ [**4**](#4-现代构建工具之ninja) 现代构建工具之Ninja
    + [**4.1**](#41-ninja的基本用法) Ninja的基本用法
    + [**4.2**](#42-ninja语法简记) Ninja语法简记
+ [**5**](#5-元构建系统之cmake) 元构建系统之CMake
    + [**5.1**](#51-cmake的基本用法) CMake的基本用法
    + [**5.2**](#52-编写脚本cmakeliststxt) 编写脚本CMakeLists.txt
    + [**5.3**](#53-常用命令commands) 常用命令（Commands）
        + [**5.3.1**](#531-基本脚本控制相关命令) 基本脚本控制相关命令
        + [**5.3.2**](#532-工程相关命令) 工程相关命令
    + [**5.4**](#54-常用内建变量variables) 常用内建变量（Variables）
+ [**6**](#6-元构建系统之autotools) 元构建系统之autotools
...
[Recipe写法](#341-recipe基本格式)

[变量](#311-变量的赋值)

[自动变量](#313-自动变量automatic-variables)

[函数](#36-使用自带函数)

[特殊目标](#326-内建特殊目标) -> [伪目标](#324-伪目标phony)

[静态规则](#329-静态格式规则static-pattern-rules)
...
由于编译技术的发展，以及软件本身的进化，程序员发现面对成千上万多种多样的源文件、库文件，手动编译一个程序变得不再现实，于是寻求一种可以根据规则自动进行文件处理的工具。这就是**Make**

Make分为很多种，**它们几乎应用在目前所有的开发环境中，已经成为目前计算机软件开发领域的重要基础设施，学习Make对于理解工程代码的管理非常重要，这和多文件大型工程的开发与管理密切相关**。除目前最常用的**GNU Make**外，FreeBSD的软件中也有使用自家开源许可证的**BSD Make**，常用的C++应用框架Qt拥有**QMake**，微软的VS有**msbuild.exe**和**nmake**，GNU Make有一个加强版**Remake**，还有常用的makefile工具**automake**和**autoconf**，近几年有流行的**Ninja**，还有强大的跨平台工具**CMake**等（类似CMake和automake这些工具**一般被称为元构建系统meta-build system**，它们不会直接和构建过程打交道，而是生成其他构建系统的脚本。Ninja有专用的元构建系统GN）。我们在一般的IDE中接触不到make，因为IDE为方便用户，往往会隐藏底层的构建过程

Make本质就是根据给定规则推导判断文件依赖，之后通过一系列处理步骤，生成或更新这些文件，一般根据文件的时间戳判断哪些文件需要更新（假设源文件时间戳比目标文件新，那么就代表这个目标文件需要重新构建）

Make几乎可以算作是通用的文件处理脚本工具，Makefile就是它的脚本。在Linux、BSD等类UNIX环境中各种强大工具的支撑下，它不仅可以用于各种语言编写的程序编译与构建，甚至可以用于图片和音频等文件的批量处理
...
但是在实际情况中，尤其是在大型工程中，这种情况会变得异常复杂。一方面，一些公司出于商业保护、专利保护等原因，或者一些软件作者单纯不想开放他的源码（这是可以理解的，比如他们不想使自己的成果在未经允许的情况下被他人修改后利用或随意进行二次发布），**他们往往只会提供一个闭源的二进制库以及一个头文件，而不是完整的源代码**。这些闭源代码只能通过**链接**应用到我们的程序中，反汇编是破译这些程序的唯一方法。另外，为了减少程序的重复，操作系统提供了动态链接技术，很多程序会使用共享的动态链接库。并且在实际中头文件和源代码的关系也是错综复杂的，为方便大型工程的管理以及简化开发（使用一个库，直接引用一个头文件即可，无需再次逐个声明函数，这个头文件同时被库和应用程序使用），实际工程中都将头文件（声明）和源文件（实现）分开
...
> **建议**
>
> 由于直接使用`*`通配符在匹配文件失败时（如文件不存在），会直接将例如`*.c`作为文件名，所以尽量避免使用`*`通配符，而使用`$(wildcard )`函数，如下例。虽然`OBJ`变量最后会展开，但是在`.c`文件不存在时会发生异常
...
| `$(addprefix prefix,names...)` | 添加前缀，示例`$(addprefix /usr/local/include/,test1.h test2.h)` |
