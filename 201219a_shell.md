...
**包含了awk编程**，使用Linux下的gawk

**包含了sed编程**，使用类Unix下的sed
...
| 值 | 名称 | 描述 |
| :-: | :-: | :-: |
| 1     | SIGHUP    | 挂起，用于通知进程其控制终端已关闭，行为可用户定义，默认行为终止 |
| 2     | SIGINT    | 普通中断信号，行为可用户定义，默认行为终止 |
| 3     | SIGQUIT   | 退出，默认行为终止并生成core dump |
| 9     | SIGKILL   | 无条件终止，不可用户定义，无处理过程 |
| 15    | SIGTERM   | 尽可能终止，可以用户定义，有处理过程 |
| 17    | SIGSTOP   | 无条件停止，不可用户定义，无处理过程 |
| 18    | SIGTSTP   | 尽可能停止，可以用户定义，有处理过程 |
| 19    | SIGCONT   | 停止后继续运行 |
