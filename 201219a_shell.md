...
正是Unix的一切皆文件的思想，**在Linux中所有对象都作为文件处理，包括输入输出进程**

文件描述符是一个非负整数，可以唯一表示会话打开的文件。Linux中一个进程可以分配9个文件描述符，其中0、1、2前3个描述符为保留，分别为`STDIN STDOUT STDERR`

**注意：标准输出和标准错误看起来都会通过控制台输出，其实是两个完全不同的输出。具体可以使用标准输出重定向符`>`测试一个命令的输出，可以发现错误信息不会重定向，依然会从控制台输出**

**默认情况下STDERR和STDOUT使用的文件描述符不同，但通常这些文件描述符都指向同一个位置**

#### 2.12.1 STDERR重定向

其实重定向符号**可以使用`n>`（n为数字）指定需要重定向的文件描述符**，这样可以使用`2>`重定向标准错误
...
如果想重定向到同一个文件，可以使用`&>`。**注意在bash中，标准错误的信息优先，会被重定向到标准输出行的上方**
