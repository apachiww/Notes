...
> 安装完成后，`ls /etc`可以发现多出了`cron.d cron.deny cron.hourly cron.daily cron.weekly cron.monthly anacrontab`等配置相关目录以及文件。其中`cron.hourly`下还有`0anacron`，`anacron`支持异步执行定时任务（解决任务由于关机而未得到执行的问题）
>
...
anacron在每个小时都会被cron调用（`/etc/cron.hourly/0anacron`）来检查并执行遗漏的任务，只能使用编辑器编辑`/etc/anacrontab`，执行频次控制到天，其默认内容一般如下（该默认配置下，需要将用户的定时脚本放置在`/etc/cron.daily`等文件夹下）
...
>
> 使用`anacron -f`可以强制触发一次所有记录的任务，`anacron -n`可以忽略延时直接执行需要执行的任务
